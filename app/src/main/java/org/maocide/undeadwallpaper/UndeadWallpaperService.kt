    ...
    class UndeadWallpaperService : WallpaperService() {
        ...
        private inner class MyWallpaperEngine : Engine() {
            ...
            override fun onSurfaceCreated(holder: SurfaceHolder) {
                ...
                // VIVO KICKSTART
                // Force the OS WindowManager to do a layout pass without locking the native buffer.
                // This tells the OEM preview picker to dismiss its black overlay safely.
                if (isPreview) {
                    Handler(Looper.getMainLooper()).post {
                        try {
                            // Re-assert OPAQUE format to trigger a window refresh safely
                            holder.setFormat(android.graphics.PixelFormat.OPAQUE)
                            FileLogger.i(TAG, "Preview kickstarted with UI format change.")
                        } catch (e: Exception) {
                            FileLogger.w(TAG, "Format kickstart failed: ${e.message}")
                        }
                    }
                }
